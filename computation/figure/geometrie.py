...
class ligne :
    def __init__(self,x0,y0,theta,d) :
        ...
        self.theta = theta
        self.d = d
        self.ID = zone.create_line([self.x0*SCALE,                                  #x1
                                    self.y0*SCALE,                                  #y1
...
class angle :
    def __init__(self,x0,y0,radius,theta0,dtheta) :
        ...
        self.theta0 = theta0
        self.dtheta = dtheta
        self.ID = zone.create_arc([SCALE*(self.x0-self.radius), #x1
                                   SCALE*(self.y0-self.radius), #y1
                                   SCALE*(self.x0+self.radius), #x2
                                   SCALE*(self.y0+self.radius)],#y2
...
class ellipse :
    def __init__(self,xf,yf,a,e,theta,morceaux=20) :
        ...
        y0 = b*np.sin(alpha)
        x = x0*np.cos(self.theta)+np.sin(theta)*y0+xf
        y = y0*np.cos(self.theta)-np.sin(theta)*x0+yf
        poly = np.ravel([x,y],'F')
        poly = poly*SCALE
